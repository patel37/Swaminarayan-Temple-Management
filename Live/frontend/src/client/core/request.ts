...
export const getHeaders = async (config: OpenAPIConfig, options: ApiRequestOptions): Promise<Record<string, string>> => {
	...
	if (isStringWithValue(token)) {
		headers['x-access-token'] = `Bearer ${localStorage.getItem('access_token')}`;
		headers['x-api-key'] = `${localStorage.getItem('api_key_user')}`;
	}

	if (isStringWithValue(username) && isStringWithValue(password)) {
		// const credentials = base64(`${username}:${password}`);
		headers['x-access-token'] = `Basic ${localStorage.getItem('access_token')}`;
		headers['x-api-key'] = `${localStorage.getItem('api_key_user')}`;
	}
